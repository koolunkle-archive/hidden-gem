// Hidden Gem - 홈 "나를 위한 추천" (로그인 시에만 노출).
// 내가 담은 가게(saved_places, RLS로 본인 것만 조회)의 최다 카테고리를 찾아 카카오 로컬
// API로 같은 카테고리를 검색한다. 이미 담은 가게는 결과에서 제외. 추천할 게 없으면 섹션을 숨긴다.
...
(function () {
  ...
  function init() {
    ...
    // auth.js 초기 세션 조회가 비동기라 리스너 등록 전에 auth-changed가 이미
    // 발생했을 수 있다 — 현재 상태도 한 번 더 확인.
