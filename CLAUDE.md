...
3. **Vercel 배포**: 이 저장소를 그대로 Vercel 프로젝트로 연결하면 정적 파일과 `api/` 폴더의 서버리스 함수가 자동으로 인식됩니다. `vercel.json`은 `framework`/`buildCommand`/`installCommand`를 전부 `null`로 명시해 빌드 단계 자체를 건너뛰도록 못 박아 두는 최소 설정만 있다 — 대시보드(Project Settings > Build and Deployment)에 과거에 남아있을 수 있는 Framework Preset/Build Command override 때문에 저장소 루트의 `server.js`가 함수로 잘못 실행되어 `index.html`이 404("Not Found: /index.html", `server.js`의 정적 서빙 404 문구)로 응답하는 문제가 있었다(2026-08-29 확인). `vercel.json`의 `null` 값이 이런 override보다 우선 적용되지만, 근본적으로는 대시보드에서도 Framework Preset이 `Other`인지, Root Directory가 저장소 루트인지 함께 확인하는 게 안전하다. Vercel 프로젝트 설정 > Environment Variables에 `KAKAO_REST_API_KEY`, `GOOGLE_PLACES_API_KEY`, `GEMINI_API_KEY`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`를 등록해야 배포 환경에서 기능이 모두 동작합니다.
